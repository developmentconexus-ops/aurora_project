...
func TestFreshProcessesRecoverSameAuroraProjectAndState(t *testing.T){
	...
	projectObj:=runJSON(t,bin,data,pass,"project","create","--label","Process Project","--objective","Fresh process continuity")
	...
	projectView:=mapField(t,shown,"project");if stringField(t,projectView,"project_id")!=pid{t.Fatalf("Project identity changed: %v",shown)}
