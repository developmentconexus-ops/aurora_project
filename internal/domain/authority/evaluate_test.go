...
func TestEvaluateScopeExpiryRevocationAndSupersession(t *testing.T) {
	...
	if got.Decision != DecisionPermitted || got.Status != EffectiveValid || got.SourceRevision != 4 {
		t.Fatalf("valid evaluation=%+v", got)
	}
	if got := Evaluate(state, "PRJ-B", "BUILD", now); got.Decision != DecisionBlocked {
		t.Fatalf("revoked grant permitted: %+v", got)
	}
	if got := Evaluate(state, "PRJ-C", "BUILD", now); got.Decision != DecisionBlocked {
		t.Fatalf("wrong scope permitted: %+v", got)
	}
	late := now.Add(2 * time.Hour)
	if got := Evaluate(state, "PRJ-A", "BUILD", late); got.Status != EffectiveExpired || got.Decision != DecisionBlocked {
		t.Fatalf("expired evaluation=%+v", got)
	}
}
...
func TestNextSafeActionIsProjectionNotGrant(t *testing.T) {
	...
	if projection.Decision != DecisionBlocked || projection.AuthorityStateRevision != 1 {
