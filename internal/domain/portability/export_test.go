...
func TestFinalizeAndVerifyExportDetectsSemanticCorruption(t *testing.T) {
	...
	final, err := Finalize(doc)
	if err != nil {
		t.Fatal(err)
	}
	if final.Integrity.PayloadSHA256 == "" {
	...
	if err := Verify(final); err == nil {
		t.Fatal("tampered logical export unexpectedly verified")
	}
}

func TestVerifyExportRejectsUnsupportedVersion(t *testing.T) {
	doc := ExportDocument{Format: "aurora-sovereign-export", Version: 2}
	if err := Verify(doc); err == nil {
		t.Fatal("unsupported export version accepted")
	}
}
