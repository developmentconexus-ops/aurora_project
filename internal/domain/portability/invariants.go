...
func Finalize(doc *Document)error{if doc.Format==""{doc.Format=FormatV1};if doc.Version==0{doc.Version=1};if doc.ExportID==""{id,err:=NewExportID();if err!=nil{return err};doc.ExportID=id};digest,err:=Digest(*doc);if err!=nil{return err};doc.Integrity.PayloadSHA256=digest;return validateSchema(*doc)}
