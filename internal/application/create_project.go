...
func (s *Service) ShowProject(ctx context.Context, pass []byte, id project.ProjectID) (ProjectView, error) {
	inspection, err := s.InspectProject(ctx, pass, id)
	if err != nil {
...
func projectFromRecord(r ports.ProjectRecord) project.Project {
	...
	if r.CurrentStateRevision != nil {
		v := project.StateRevision(*r.CurrentStateRevision)
		rev = &v
	}
