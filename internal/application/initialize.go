...
func (s *Service) Initialize(ctx context.Context, passphrase []byte) (InitializeResult, error) {
	...
	if _, err := s.Trust.LoadRootEnvelope(ctx); err == nil {
		return InitializeResult{}, ErrAlreadyInitialized
	} else if !errors.Is(err, fs.ErrNotExist) {
		return InitializeResult{}, fmt.Errorf("check existing owner root: %w", err)
	}
	env, ork, err := newRootEnvelope(passphrase)
	if err != nil { return InitializeResult{}, err }
	defer zero(ork)
	...
	authority := map[string]any{
		"revision": uint64(1), "predecessor_revision": nil, "revalidation_required": false,
		"grants": []any{map[string]any{
			"authority_id":"AUTH-OWNER-INITIAL", "authority_revision":uint64(1), "subject_operator_id":"OWNER-LEANDRO",
			"actor_id":"AURORA_CORE", "project_scope":[]string{"*"},
			"permitted_action_classes":[]string{"project.create","project.state.transition","authority.admin","export","restore","migration"},
			"lifecycle_status":"ACTIVE", "issued_at":now, "provenance":"owner-bootstrap",
		}},
	}
	authorityJSON, err := json.Marshal(authority)
	if err != nil { return InitializeResult{}, err }
	mac, err := governingMAC(ork, id, 1, authorityJSON)
	if err != nil { return InitializeResult{}, err }
	_, err = s.State.Bootstrap(ctx, ports.BootstrapMutation{Identity:id, AuthorityStateJSON:authorityJSON, CurrentAuthorityRevision:1, GoverningGeneration:1, GoverningMAC:mac})
	if err != nil { return InitializeResult{}, fmt.Errorf("bootstrap state: %w", err) }
	...
	if err != nil { return InitializeResult{}, err }
	anchor.HMAC = encodeMAC(anchorMACBytes)
