...
type bootstrapGoverningSnapshot struct {
	Version             int                        `json:"version"`
	Aurora              bootstrapAurora            `json:"aurora"`
	GoverningGeneration uint64                     `json:"governing_generation"`
	Projects            []governingProjectSnapshot `json:"projects"`
	Authority           json.RawMessage            `json:"authority"`
}
type bootstrapAurora struct {
	AuroraID        string `json:"aurora_id"`
	OwnerOperatorID string `json:"owner_operator_id"`
}

func (s *Service) Initialize(ctx context.Context, passphrase []byte) (InitializeResult, error) {
	if s.State == nil || s.Trust == nil || s.Clock == nil {
		return InitializeResult{}, errors.New("service dependencies are incomplete")
	}
	current, stateErr := s.State.LoadCurrent(ctx)
	root, rootErr := s.Trust.LoadRootEnvelope(ctx)
	stateExists := stateErr == nil
	rootExists := rootErr == nil
	if stateExists || rootExists {
		if stateExists && rootExists {
			return InitializeResult{}, ports.ErrAlreadyInitialized
		}
		return InitializeResult{}, ErrIncompleteBootstrap
	}
	if !errors.Is(stateErr, ports.ErrNotInitialized) {
		return InitializeResult{}, stateErr
	}
	if !errors.Is(rootErr, ports.ErrRootNotFound) {
		return InitializeResult{}, rootErr
	}
	_ = current
	_ = root
	env, ork, err := newRootEnvelope(passphrase)
	if err != nil {
		return InitializeResult{}, err
	}
	defer clear(ork)
	if err := s.Trust.StoreRootEnvelope(ctx, env); err != nil {
		return InitializeResult{}, fmt.Errorf("publish owner root: %w", err)
	}
	auroraID, err := identity.NewAuroraID()
	if err != nil {
		return InitializeResult{}, err
	}
	now := s.Clock.Now().UTC()
	authorityID, err := randomIdentifier("AUTH-")
	if err != nil {
		return InitializeResult{}, err
	}
	...
	mac, err := governingMAC(ork, snapshot)
	if err != nil {
		return InitializeResult{}, err
	}
	operationID, err := randomIdentifier("OP-")
	if err != nil {
		return InitializeResult{}, err
	}
	if _, err := s.State.Bootstrap(ctx, ports.BootstrapMutation{OperationID: operationID, AuroraID: string(auroraID), OwnerOperatorID: ownerOperatorID, CreatedAt: now, AuthorityRevision: 1, AuthorityJSON: authorityJSON, GoverningGeneration: 1, GoverningDescriptorHMAC: mac}); err != nil {
		return InitializeResult{}, err
	}
	anchor := ports.Anchor{Version: 1, RootID: env.RootID, AuroraID: string(auroraID), GoverningGeneration: 1, ObservedWallTimeHighWater: now}
	...
	if err := s.Trust.PublishAnchor(ctx, anchor); err != nil {
		return InitializeResult{}, fmt.Errorf("publish owner anchor: %w", err)
	}
