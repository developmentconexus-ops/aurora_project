...
func (s *Service) ShowAuthority(ctx context.Context, pass []byte, projectID project.ProjectID, action string) (authority.Snapshot, error) {
	...
	state, err := decodeAuthorityState(assessment.Current.AuthorityJSON)
	if err != nil {
		return authority.Snapshot{}, err
	}
	if assessment.State != TrustNormal {
		decision := authority.DecisionBlocked
		status := authority.EffectiveInvalid
		if assessment.State == TrustRevalidationRequired {
			decision = authority.DecisionRevalidationRequired
			status = authority.EffectiveRevalidationRequired
		}
		return authority.Snapshot{SourceRevision: state.Revision, Decision: decision, Status: status, Reasons: []string{string(assessment.State)}, EvaluatedAt: s.Clock.Now().UTC()}, nil
	}
...
func (s *Service) GrantAuthority(ctx context.Context, pass []byte, in GrantAuthorityInput) (GrantAuthorityResult, error) {
	...
	if _, err := s.requireNormalTrust(ctx, pass); err != nil {
