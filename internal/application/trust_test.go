...
func TestClassifyTrustTable(t *testing.T) {
	...
	cases:=[]struct{name string;in trustSignals;want TrustState}{
	...
	for _,tc:=range cases{t.Run(tc.name,func(t *testing.T){if got:=classifyTrust(tc.in);got!=tc.want{t.Fatalf("got %s want %s",got,tc.want)}})}
}
...
func TestGoverningMACBindsCurrentStateContentNotOnlyRevision(t *testing.T){
	...
	mac1,err:=governingMAC(key,governingSnapshot(base));if err!=nil{t.Fatal(err)}
	base.Projects[0].CurrentState.State.Payload=[]byte(`{"x":2}`)
	mac2,err:=governingMAC(key,governingSnapshot(base));if err!=nil{t.Fatal(err)}
	if bytes.Equal(mac1,mac2){t.Fatal("governing MAC did not change when current state content changed at same revision")}
}
