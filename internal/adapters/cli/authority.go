...
func runAuthority(ctx context.Context, s *application.Service, pass []byte, args []string, out, errOut io.Writer, asJSON bool) int {
	if len(args) == 0 {
		return usageError(errOut, "authority requires show, grant, revoke or reconcile-anchor")
	}
	switch args[0] {
	case "show":
	...
	case "reconcile-anchor":
		if len(args) != 1 {
			return usageError(errOut, "reconcile-anchor takes no arguments")
		}
		r, err := s.ReconcileAnchor(ctx, pass)
		if err != nil {
			return runtimeError(errOut, err)
		}
		if err := renderInspect(out, r, asJSON); err != nil {
			return runtimeError(errOut, err)
		}
		return 0
	default:
