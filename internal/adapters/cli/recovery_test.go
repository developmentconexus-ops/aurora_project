...
func TestRecoverCurrentStateReconstructsProjectAuthorityAndNextAction(t *testing.T) {
	...
	if _, code, e := run("authority", "grant", "--project", pid, "--action", "BUILD"); code != 0 {
		t.Fatal(e)
	}

	recovered, code, e := run("recover")
	if code != 0 {
		t.Fatalf("recover code=%d err=%q", code, e)
	}
	if recovered["status"] != "RECOVERED" || recovered["trust_state"] != "NORMAL" {
	...
	if projectView["project_id"] != pid || projectView["current_state_revision"].(float64) != 1 {
		t.Fatalf("project recovery=%v", projectView)
	}
	projection := projectView["next_safe_action"].(map[string]any)
	if projection["decision"] != "PERMITTED" {
		t.Fatalf("next safe action=%v", projection)
	}
}
...
func TestRecoverCurrentStateDoesNotPromoteHistoryWhenCurrentPointerIsBroken(t *testing.T) {
	...
	out.Reset()
	errOut.Reset()
	if code := runWithSecretReader([]string{"--json", "--data-dir", dataDir, "project", "create", "--label", "Broken", "--objective", "Broken pointer"}, &out, &errOut, secrets); code != 0 {
		t.Fatal(errOut.String())
	}
	...
	out.Reset()
	errOut.Reset()
	if code := runWithSecretReader([]string{"--json", "--data-dir", dataDir, "project", "set-state", "--project", pid, "--expected", "none", "--kind", "WORK_NOTE", "--summary", "R1", "--payload", `{}`}, &out, &errOut, secrets); code != 0 {
		t.Fatal(errOut.String())
	}
	...
	if _, err := db.Exec(`UPDATE projects SET current_state_revision=99 WHERE project_id=?`, pid); err != nil {
		t.Fatal(err)
	}
	...
	if code != 0 {
		t.Fatalf("recover diagnostic code=%d err=%q", code, errOut.String())
	}
	var result map[string]any
	if err := json.Unmarshal(out.Bytes(), &result); err != nil {
		t.Fatal(err)
	}
	if result["status"] != "FAILED" || result["classification"] != "DURABLE_STATE_INTEGRITY_FAILURE" {
		t.Fatalf("result=%v", result)
	}
	if projects, ok := result["projects"].([]any); ok && len(projects) != 0 {
		t.Fatalf("fabricated/promoted projects=%v", projects)
	}
}
