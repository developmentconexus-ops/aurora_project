...
func TestAuthorityGrantRestartRevokeAndProjection(t *testing.T) {
	...
	if code != 0 { t.Fatal(e) }
	if before["decision"] != "BLOCKED" { t.Fatalf("before grant=%v", before) }
	...
	if code != 0 { t.Fatal(e) }
	if after["decision"] != "PERMITTED" { t.Fatalf("after grant=%v", after) }
	if _, code, e := run("authority", "revoke", "--authority", aid); code != 0 { t.Fatal(e) }
	revoked, code, e := run("authority", "show", "--project", pid, "--action", "BUILD")
	if code != 0 { t.Fatal(e) }
	if revoked["decision"] != "BLOCKED" { t.Fatalf("after revoke=%v", revoked) }
}
