...
func runProject(ctx context.Context, s *application.Service, pass []byte, args []string, out, errOut io.Writer, asJSON bool) int {
	...
	case "create":
		...
		if err := renderProject(out, application.ProjectInspection{Project:p}, asJSON); err != nil { return runtimeError(errOut, err) }; return 0
	case "show":
		...
		inspection, err := s.InspectProject(ctx, pass, project.ProjectID(id)); if err != nil { return runtimeError(errOut, err) }
		if err := renderProject(out, inspection, asJSON); err != nil { return runtimeError(errOut, err) }; return 0
