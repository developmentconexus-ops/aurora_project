...
Usage:
  ...
  aurora [--json] migrate --in <old.aurora.age> --out <new.aurora.age>
  aurora --help
`
...
func Run(args []string, out, errOut io.Writer) int {
	...
	if rest[0] == "migrate" {
		return runMigrate(rest[1:], opts, out, errOut)
	}
	if rest[0] != "restore" && opts.dataDir == "" {
	...
	return runCommand(rest, opts, out, errOut)
}
...
func parseGlobals(args []string) (globalOptions, []string, error) {
	...
	for i := 0; i < len(args); i++ {
		...
		case "--data-dir":
			if i+1 >= len(args) {
				return opts, nil, fmt.Errorf("--data-dir requires a path")
			}
			i++
			opts.dataDir = args[i]
		case "--json":
...
func defaultDataDir() (string, error) {
	if value := os.Getenv("AURORA_DATA_DIR"); value != "" {
		return value, nil
	}
