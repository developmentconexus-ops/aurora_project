...
type transitionAuditMetadata struct {
	Source                 string                 `json:"source"`
	Classification         string                 `json:"classification"`
	ActorID                string                 `json:"actor_id"`
	ExpectedStateRevision  *project.StateRevision `json:"expected_state_revision,omitempty"`
	AuthorityEvaluationRef string                 `json:"authority_evaluation_ref,omitempty"`
}
...
func (s *Store) CommitProjectTransition(ctx context.Context, in ports.ProjectTransitionMutation) (ports.ProjectTransitionResult, error) {
	...
	if generation != in.ExpectedGeneration {
		return ports.ProjectTransitionResult{}, ErrConcurrentMutation
	}
	if !dbRevisionMatches(current, in.ExpectedRevision) {
		if err := recordRejectedTransitionTx(ctx, tx, in, "STALE_REVISION"); err != nil {
			return ports.ProjectTransitionResult{}, err
		}
	...
	auditDetails, err := transitionAuditDetails(in.ActorID, in.ExpectedRevision, in.AuthorityEvaluationRef)
	if err != nil {
		return ports.ProjectTransitionResult{}, err
	}
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO records(record_id,kind,operation_id,project_id,state_revision,outcome,reason,details_json,created_at) VALUES(?,?,?,?,?,?,?,?,?)`, "REC-"+in.AttemptID, "AUDIT", in.AttemptID, in.ProjectID, in.Revision.Revision, "ACCEPTED", "STATE_TRANSITION", auditDetails, in.Revision.AcceptedAt.UTC().Format(time.RFC3339Nano)); err != nil {
		return ports.ProjectTransitionResult{}, err
	}
	if err := tx.Commit(); err != nil {
		return ports.ProjectTransitionResult{}, err
...
func (s *Store) RecordNonGoverning(ctx context.Context, in ports.RecordMutation) error {
	...
	_, err = tx.ExecContext(ctx, `INSERT INTO records(record_id,kind,operation_id,project_id,outcome,reason,details_json,created_at) VALUES(?,?,?,?,?,?,?,?)`, "REC-"+in.AttemptID, "AUDIT", in.AttemptID, in.ProjectID, in.Result, in.Reason, auditDetails, in.RequestedAt.UTC().Format(time.RFC3339Nano))
	if err != nil {
		return err
	}
	return tx.Commit()
...
func recordRejectedTransitionTx(ctx context.Context, tx *sql.Tx, in ports.ProjectTransitionMutation, reason string) error {
	...
	_, err = tx.ExecContext(ctx, `INSERT INTO records(record_id,kind,operation_id,project_id,outcome,reason,details_json,created_at) VALUES(?,?,?,?,?,?,?,?)`, "REC-"+in.AttemptID, "AUDIT", in.AttemptID, in.ProjectID, "REJECTED", reason, auditDetails, at)
	return err
}

func transitionAuditDetails(actorID string, expected *project.StateRevision, authorityRef string) (string, error) {
	raw, err := json.Marshal(transitionAuditMetadata{
		Source:                 "aurora-sovereign-core",
		Classification:         "PROJECT_TRANSITION",
		ActorID:                actorID,
		ExpectedStateRevision:  expected,
		AuthorityEvaluationRef: authorityRef,
	})
	if err != nil {
		return "", err
	}
...
func nullableRevision(v *project.StateRevision) any {
	if v == nil {
		return nil
	}
	return int64(*v)
