...
func (s *Store) loadProjects(ctx context.Context) ([]ports.ProjectRecord, error) {
	...
	for rows.Next() {
		...
		var rev sql.NullInt64
		var created, updated string
		if err := rows.Scan(&r.ProjectID,&r.DisplayLabel,&r.ObjectiveSummary,&rev,&created,&updated); err != nil { return nil, err }
		if rev.Valid { v := uint64(rev.Int64); r.CurrentStateRevision=&v }
		r.CreatedAt, err = time.Parse(time.RFC3339Nano, created); if err != nil { return nil, err }
		r.UpdatedAt, err = time.Parse(time.RFC3339Nano, updated); if err != nil { return nil, err }
		out=append(out,r)
	}
	return out, rows.Err()
}
