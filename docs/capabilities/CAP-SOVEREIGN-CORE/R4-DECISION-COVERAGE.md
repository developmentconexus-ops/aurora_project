...
R3 identified exactly 15 implementation-blocking R4 questions. R4 must make every question discoverably resolved, intentionally deferred by a named trigger, or blocked by explicit missing evidence.

```text
R3 R4 questions expected: 15
...
|---|---|---|---|---|---|
| `R4-Q-CORE-001` Core runtime | HIGH_LOCK_IN | RUNTIME-PERSISTENCE-R4 | ADR-0003 → Go | language-specific spike not required | `PROPOSED_ADR_READY / OPERATOR_REVIEW_REQUIRED` |
...
| `R4-Q-TOPOLOGY-001` process topology | NO_REGRET_M0 | RUNTIME-PERSISTENCE-R4 | ADR-0004 → one local modular Core process | no topology spike required for M0 | `PROPOSED_ADR_READY / OPERATOR_REVIEW_REQUIRED` |
| `R4-Q-ENGINE-001` durable engine | NOT_YET_A_DECISION | RUNTIME-PERSISTENCE-R4 + A0 DURABLE-EXECUTION research | ADR-0004 → do not introduce in M0; reconsider at M4/current requirement trigger | no M0 engine spike | `INTENTIONAL_NON_SELECTION / OPERATOR_REVIEW_REQUIRED` |
...
## 3. Proposed decisions ready for operator review without further architecture spike

```text
ADR-0003 — Go as initial Core runtime
ADR-0004 — local modular Core + explicit current state/revisions + no event sourcing/durable engine in M0
...
## 4. Decisions that cannot be accepted yet
...
## 5. R4 gate implication

At this documentary checkpoint:

```text
all 15 questions are accounted for
BUT
2 material Architecture Spikes require execution/review
AND
6 proposed ADRs remain unaccepted
```
...
Blockers are explicit and narrow:
...
4. operator review/acceptance is required for material ADRs after evidence is sufficient.
